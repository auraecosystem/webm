...
static int vpx_img_plane_width(const vpx_image_t *img, size_t plane) {
...
static int vpx_img_plane_height(const vpx_image_t *img, size_t plane) {
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  ...
  // We may want to add more config options (for more complex encoders as seen in the examples) in the future
  // while still maintaining the same format (so that generated corpus is still valid).
  // So we reserve FUZZ_HDR_SZ=32 bytes for this even if we just use one byte so far.
  data += FUZZ_HDR_SZ;
  size -= FUZZ_HDR_SZ;
