...
void vp9_rc_set_frame_target(VP9_COMP *cpi, int target) {
  ...
  const int64_t sb64_target_rate =
      ((int64_t)rc->this_frame_target * 64 * 64) / ((int64_t)cm->width * cm->height);
...
int vp9_resize_one_pass_cbr(VP9_COMP *cpi) {
  ...
  if (((int64_t)cm->width * cm->height) < min_width * min_height) down_size_on = 0;
  ...
  // for HD input.
  if (cpi->resize_state == ORIG && (int64_t)cm->width * cm->height >= 1280 * 720) {
