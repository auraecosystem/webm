...
static int rc_pick_q_and_bounds_one_pass_vbr(const VP9_COMP *cpi,
  ...
  *bottom_index = active_best_quality;
  ...
  if (oxcf->rc_mode == VPX_Q) {
    ...
    // Note the buffer_level is updated for every encoded frames as:
    // buffer_level - starting_buffer_level += (avg_frame_bandwidth - encoded_frame_size).
    // So normalizing this with framerate and #encoded frames (currrent_video_frame)
    // gives the difference/error between target and encoding bitrate.
    // The additional avg_frame_bandwidth term is to compensated for the pre-encoded
    // buffer update (in vp9_rc_get_one_pass_vbr_params).
    int bitrate_err = (int)(cpi->framerate *
                          (rc->buffer_level - rc->starting_buffer_level -
