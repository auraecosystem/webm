...
static void set_rt_speed_feature_framesize_independent(
  ...
  if (speed >= 5) {
    ...
    if ((int64_t)cm->width * cm->height > 1280 * 720) sf->cb_pred_filter_search = 2;
  ...
  if (speed >= 7) {
    ...
    if ((int64_t)cm->width * cm->height > 640 * 480) sf->cb_pred_filter_search = 2;
  }
  ...
  if (speed >= 8) {
    ...
    if (!frame_is_intra_only(cm) && (int64_t)cm->width * cm->height <= 352 * 288) {
    ...
    sf->limit_newmv_early_exit = 0;
    sf->use_simple_block_yrd = 1;
    if ((int64_t)cm->width * cm->height > 352 * 288) sf->cb_pred_filter_search = 2;
  }
  ...
  if (speed >= 9) {
    ...
    if ((int64_t)cm->width * cm->height >= 640 * 360) sf->variance_part_thresh_mult = 2;
  }
  ...
  if (cm->frame_type != KEY_FRAME && (int64_t)cm->width * cm->height <= 320 * 240 &&
      sf->partition_search_type == VAR_BASED_PARTITION &&
