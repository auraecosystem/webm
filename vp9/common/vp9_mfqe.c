...
static void mfqe_partition(VP9_COMMON *cm, MODE_INFO *mi, BLOCK_SIZE bs,
  ...
  // This avoids reading uninitialized/invalide block sizes outside the frame.
  // Return also for blocks smaller than 8x8, and note 4x4 sublocks can occur
  // inside the image (not only on boundary).
  // The return here is safe as the pixels are pre-copied by vpx_yv12_copy_frame
  // in vp9_mfqe() before entering the superblock loop. Without pre-copying,
  // returning here would leave these blocks uncopied/uninitialized in the
  // destination buffer.
