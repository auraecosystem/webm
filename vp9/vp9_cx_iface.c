...
typedef struct vp9_extracfg {
  ...
  int delta_q_uv;
  unsigned int validate_input_hdb;
} vp9_extracfg;
...
static vpx_codec_err_t ctrl_set_validate_input_hdb(vpx_codec_alg_priv_t *ctx,
                                                  va_list args) {
  struct vp9_extracfg extra_cfg = ctx->extra_cfg;
  extra_cfg.validate_input_hdb = CAST(VP9E_SET_VALIDATE_INPUT_HDB, args);
  return update_extra_cfg(ctx, &extra_cfg);
}
...
#if CONFIG_VP9_HIGHBITDEPTH
      if (ctx->extra_cfg.validate_input_hdb &&
          (img->fmt & VPX_IMG_FMT_HIGHBITDEPTH) &&
          ctx->oxcf.input_bit_depth > 8) {
        const int h = (int)img->d_h;
        const int w = (int)img->d_w;
        const int bit_depth = (int)ctx->oxcf.input_bit_depth;
        const int max_val = 1 << bit_depth;
        int plane;
        for (plane = 0; plane < 3; ++plane) {
          const unsigned short *src = (const unsigned short *)img->planes[plane];
          const int stride = img->stride[plane] / 2;
          const int ph = (plane == 0)
                             ? h
                             : (int)((h + img->y_chroma_shift) >> img->y_chroma_shift);
          const int pw = (plane == 0)
                             ? w
                             : (int)((w + img->x_chroma_shift) >> img->x_chroma_shift);
          int i, j;
          for (i = 0; i < ph; ++i) {
            for (j = 0; j < pw; ++j) {
              if (src[i * stride + j] >= max_val) {
                return VPX_CODEC_INVALID_PARAM;
              }
            }
          }
...
static vpx_codec_ctrl_fn_map_t encoder_ctrl_maps[] = {
  ...
  { VP9E_SET_VALIDATE_INPUT_HDB, ctrl_set_validate_input_hdb },
