...
/*
 ...
 lo: lo_hi | lo_lo
*/
#define ALIGNR_256(hi, lo, i)                              \
  ({                                                       \
    __m256i _hi = (hi);                                    \
    __m256i _lo = (lo);                                    \
    __m256i _mid = _mm256_permute2x128_si256(_hi, _lo, 3); \
    _mm256_alignr_epi8(_mid, _lo, (i));                    \
  })
...
void vpx_highbd_d63_predictor_32x32_avx2(uint16_t *dst, ptrdiff_t stride,
  ...
  __m256i B0 = ALIGNR_256(A1, A0, 2);
  __m256i B1 = ALIGNR_256(AR, A1, 2);

  __m256i C0 = ALIGNR_256(A1, A0, 4);
  __m256i C1 = ALIGNR_256(AR, A1, 4);
  ...
  for (int i = 0; i < 30; i += 2) {
    ...
    avg2_0 = ALIGNR_256(avg2_1, avg2_0, 2);
    avg2_1 = ALIGNR_256(AR, avg2_1, 2);
    avg3_0 = ALIGNR_256(avg3_1, avg3_0, 2);
    avg3_1 = ALIGNR_256(AR, avg3_1, 2);
  }
