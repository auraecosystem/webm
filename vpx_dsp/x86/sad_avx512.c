...
static INLINE unsigned int sad32xh_avx512(const uint8_t *src_ptr,
  ...
  const int max = h >> 1;
  for (i = 0; i < max; i++) {
    ref1_reg = _mm256_loadu_si256((const __m256i *)ref_ptr);
    ref2_reg = _mm256_loadu_si256((const __m256i *)(ref_ptr + ref_stride));
    sad1_reg =
        _mm256_sad_epu8(ref1_reg, _mm256_loadu_si256((const __m256i *)src_ptr));
    sad2_reg = _mm256_sad_epu8(
        ref2_reg, _mm256_loadu_si256((const __m256i *)(src_ptr + src_stride)));
...
#define FSAD32_H(h)                                                           \
  unsigned int vpx_sad32x##h##_avx512(const uint8_t *src_ptr, int src_stride, \
                                      const uint8_t *ref_ptr,                 \
                                      int ref_stride) {                       \
    return sad32xh_avx512(src_ptr, src_stride, ref_ptr, ref_stride, h);       \
  }

#define FSADS32_H(h)                                                  \
  unsigned int vpx_sad_skip_32x##h##_avx512(                          \
      const uint8_t *src_ptr, int src_stride, const uint8_t *ref_ptr, \
      int ref_stride) {                                               \
    return 2 * sad32xh_avx512(src_ptr, src_stride * 2, ref_ptr,       \
                              ref_stride * 2, h / 2);                 \
  }
...
#define FSADAVG32_H(h)                                                        \
  unsigned int vpx_sad32x##h##_avg_avx512(                                    \
      const uint8_t *src_ptr, int src_stride, const uint8_t *ref_ptr,         \
      int ref_stride, const uint8_t *second_pred) {                           \
    int i;                                                                    \
    __m256i sad1_reg, sad2_reg, ref1_reg, ref2_reg;                           \
    ...
    for (i = 0; i < max; i++) {                                               \
      ...
      ref1_reg = _mm256_avg_epu8(                                             \
          ref1_reg, _mm256_loadu_si256((const __m256i *)second_pred));        \
      ref2_reg = _mm256_avg_epu8(                                             \
          ref2_reg, _mm256_loadu_si256((const __m256i *)(second_pred + 32))); \
      sad1_reg = _mm256_sad_epu8(                                             \
          ref1_reg, _mm256_loadu_si256((const __m256i *)src_ptr));            \
      sad2_reg = _mm256_sad_epu8(                                             \
          ref2_reg,                                                           \
          _mm256_loadu_si256((const __m256i *)(src_ptr + src_stride)));       \
      sum_sad =                                                               \
