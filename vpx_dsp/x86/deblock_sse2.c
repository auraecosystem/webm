...
void vpx_post_proc_down_and_across_mb_row_sse2(unsigned char *src,
  ...
  for (row = 0; row < size; row++) {
    for (col = 0; col < cols; col += 16) {
      ...
      const __m128i out = filter_16_sse2(v, p_above2, p_above1, p_below1,
                                         p_below2, f, zero);
    ...
    for (col = 0; col < cols; col += 16) {
      ...
      const __m128i p_left2 =
          _mm_loadu_si128((const __m128i *)(dst + col - 2));
      const __m128i p_left1 =
  ...
  for (r = 0; r < rows; r++) {
    ...
    for (c = 0; c < cols + 8; c += 8) {
      ...
      if (c < cols) {
        ...
        const __m128i res32_lo = filter_4_across(
            s + c, left_lo, right_lo, &sum, &sumsq, f_vec, eight, zero);
