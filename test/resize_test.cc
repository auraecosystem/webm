...
void ScaleForFrameNumber(unsigned int frame, unsigned int initial_w,
  ...
  *h = initial_h;
  ...
  if (random_input_one_half_only_ == 1) {
    if (frame < 100) {
      return;
    }
    *w = initial_w / 2;
...
class ResizingVideoSource : public ::libvpx_test::DummyVideoSource {
 public:
  ResizingVideoSource() {
    ...
    random_input_one_half_only_ = false;
    top_width_ = kInitialWidth;
    top_height_ = kInitialHeight;
  }
  bool flag_codec_;
  bool smaller_width_larger_size_;
  bool random_input_one_half_only_;
...
// This tests used 4 threads with smal keyframe spacing, and
// random input, and uses 640x480 as initial resolution.
TEST_P(ResizeRealtimeTest, TestExternalResizeWorks4threads) {
  ResizingVideoSource video;
  video.flag_codec_ = true;
  video.smaller_width_larger_size_ = false;
  video.random_input_one_half_only_ = true;
  video.top_width_ = 640;
  video.top_height_ = 480;
  DefaultConfig();
  // Disable internal resize for this test.
  cfg_.rc_resize_allowed = 0;
  cfg_.g_threads = 4;
  ...
  for (std::vector<FrameInfo>::const_iterator info = frame_info_list_.begin();
    ...
    unsigned int expected_w;
    unsigned int expected_h;
    ScaleForFrameNumber(frame, video.top_width_, video.top_height_, &expected_w,
                        &expected_h, video.flag_codec_,
