...
TEST(EncodeAPI, MultiResEncodeFirstIterFailOOB) {
  ...
  for (int i = 0; i < kNumEnc; ++i) {
    ASSERT_EQ(VPX_CODEC_OK,
              vpx_codec_enc_config_default(&vpx_codec_vp8_cx_algo, &cfg[i], 0));
    cfg[i].g_w = w[i];
    cfg[i].g_h = h[i];
  ...
  ASSERT_EQ(VPX_CODEC_OK,
            vpx_codec_enc_init_multi(enc.data(), &vpx_codec_vp8_cx_algo, cfg,
                                     kNumEnc, 0, dsf));
  ...
  for (int i = 0; i < kNumEnc; ++i) {
    ASSERT_NE(nullptr,
              vpx_img_alloc(&imgs[i], VPX_IMG_FMT_I420, w[i], h[i], 1));
  ...
  vpx_codec_encode(enc.data(), imgs, /*pts=*/0, /*duration=*/1,
                   /*flags=*/0, VPX_DL_REALTIME);
