...
TEST(EncodeAPI, Buganizer442105459) {
  ...
  const vpx_enc_deadline_t frame_deadlines[] = { (vpx_enc_deadline_t)1,
                                                 (vpx_enc_deadline_t)1 };
  for (int frame = 0; frame < num_frames; frame++) {
    ...
    ASSERT_EQ(vpx_codec_encode(&ctx, img, pts, duration, flags, deadline),
              VPX_CODEC_OK);
  }
  // Flush encoder.
