...
TEST(EncodeAPI, PerceptualAQKMeansHeapOverflow) {
  ...
  vpx_image_t *img = CreateImage(VPX_BITS_8, VPX_IMG_FMT_I420, cfg.g_w, cfg.g_h);
  ASSERT_NE(img, nullptr);

  ASSERT_EQ(vpx_codec_encode(&enc, img, 0, 1, 0, VPX_DL_REALTIME), VPX_CODEC_OK);

  ASSERT_EQ(vpx_codec_control(&enc, VP9E_SET_AQ_MODE, PERCEPTUAL_AQ), VPX_CODEC_OK);
  ...
  ASSERT_EQ(vpx_codec_encode(&enc, img, 1, 1, 0, VPX_DL_REALTIME), VPX_CODEC_OK);
