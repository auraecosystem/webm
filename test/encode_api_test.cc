...
// This is a regression test for the SSIM tuning heap-buffer-overflow during
// resolution change.
TEST(EncodeAPI, BuganizerSSIMHeapOverflow) {
  ...
  // Change config to larger size
  cfg.g_w = 4096;
  cfg.g_h = 4096;
  ASSERT_EQ(vpx_codec_enc_config_set(&enc, &cfg), VPX_CODEC_OK);

  vpx_img_free(img);
  img = vpx_img_alloc(nullptr, VPX_IMG_FMT_I420, cfg.g_w, cfg.g_h, 1);
  ASSERT_NE(img, nullptr);

  // Encode second frame - this used to crash
