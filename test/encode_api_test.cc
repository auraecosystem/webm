...
#if CONFIG_VP8_ENCODER
TEST(EncodeAPI, VP8SetReferenceSizeValidation) {
  vpx_codec_iface_t *const iface = vpx_codec_vp8_cx();
  vpx_codec_enc_cfg_t cfg;
  vpx_codec_ctx_t enc;
  ASSERT_EQ(VPX_CODEC_OK, vpx_codec_enc_config_default(iface, &cfg, 0));

  cfg.g_w = 128;
  cfg.g_h = 128;

  ASSERT_EQ(VPX_CODEC_OK, vpx_codec_enc_init(&enc, iface, &cfg, 0));

  vpx_image_t raw;
  ASSERT_NE(nullptr, vpx_img_alloc(&raw, VPX_IMG_FMT_I420, 128, 128, 1));
  ASSERT_EQ(VPX_CODEC_OK,
            vpx_codec_encode(&enc, &raw, 0, 1, 0, VPX_DL_REALTIME));

  // Create a larger reference frame.
  vpx_image_t ref_img;
  ASSERT_NE(nullptr, vpx_img_alloc(&ref_img, VPX_IMG_FMT_I420, 256, 256, 1));
  ...
  EXPECT_EQ(VPX_CODEC_INVALID_PARAM,
            vpx_codec_control(&enc, VP8_SET_REFERENCE, &ref_frame));

  // Copying reference frame with incorrect size must also fail.
  EXPECT_EQ(VPX_CODEC_INVALID_PARAM,
            vpx_codec_control(&enc, VP8_COPY_REFERENCE, &ref_frame));
...
#if CONFIG_VP9_ENCODER
TEST(EncodeAPI, VP9SetReferenceSizeValidation) {
  vpx_codec_iface_t *const iface = vpx_codec_vp9_cx();
  vpx_codec_enc_cfg_t cfg;
  vpx_codec_ctx_t enc;
  ASSERT_EQ(VPX_CODEC_OK, vpx_codec_enc_config_default(iface, &cfg, 0));
